...
| 1.5 | Secret leakage in bundle | NO — no secrets ship in prod | Fixed by [#118](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/118) (commit `1c2d1bb`): literal `DEFAULT_DEV_MNEMONIC` removed from `devWallet.ts`; `VITE_DEV_MNEMONIC` is now required at runtime when `VITE_DEV_MODE=true`; production `vite build` throws if `VITE_DEV_MNEMONIC` is set in any merged `.env*` or shell. Defence-in-depth: gitleaks rule `bip39-like-phrase-frontend-src` flags 12+ quoted lowercase 3-8 letter words in `frontend-dapp/src/*.ts(x)` (allowlists tests + Terra addresses). Verified 2026-05-01 — clean prod build OK, `VITE_DEV_MNEMONIC=x npm run build` fails with the #118 error, `grep -rF 'notice oak worry' dist/assets/*.js` empty, gitleaks regex confirmed against the original literal. | OK |
